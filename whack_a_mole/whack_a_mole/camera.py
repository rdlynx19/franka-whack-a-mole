...
class Camera(Node):
    ...
    def __init__(self):
        ...
        self.robot_state = ROBOT_STATE.STATIONARY
        self.led_on = None
    ...
    def detect_illumination(self, color):
        ...
        Uses background subtraction to detect significant changes.
        Adds text to the image indicating the state of the illumination.
        """
        color_index = COLORS[color]

        # Get the current and previous center pixel positions
        color_pixel_yx = np.array([np.mean(self.running_avg[color_index][:, 1]), 
                                   np.mean(self.running_avg[color_index][:, 0])], dtype=int)

        # Current RGB value at the center pixel
        rgb_value = self.color_image[color_pixel_yx[0], color_pixel_yx[1]]

        # Current average RGB value (running average)
        curr_average = np.mean(self.rgb_running_avg[color_index][:, 0]), \
                       np.mean(self.rgb_running_avg[color_index][:, 1]), \
                       np.mean(self.rgb_running_avg[color_index][:, 2])
        curr_average = np.array(curr_average, dtype=int)

        # Compute the difference (background subtraction)
        difference = np.linalg.norm(rgb_value - curr_average)
        
        # Check if the light is illuminated
        if difference >= self.illumination_threshold:
            state_text = f"{color} ILLUMINATED!!!!"
            msg = String()
            msg.data = color
            if self.led_on != color:
                self.move_pub.publish(msg=msg)
                self.led_on = color
                self.log(color, state_text)

            # Update the running average
            self.rgb_running_avg[color_index, :-1] = self.rgb_running_avg[color_index, 1:]  # Shift elements
            ...
            # Update the running average
            self.rgb_running_avg[color_index, :-1] = self.rgb_running_avg[color_index, 1:]
            self.rgb_running_avg[color_index, -1] = np.array(rgb_value)

        # Overlay the state text on the image
        font = cv2.FONT_HERSHEY_SIMPLEX
        text_position = (50, 50)  # Position of the text on the image (you can adjust this)
        font_scale = 1
        font_color = (0, 255, 0)  # Green color (you can change it)
        thickness = 2
        ...
        # Optionally, you can display the image for testing
        # cv2.imshow('LED Detection', self.color_image)
        # cv2.waitKey(1)

        # Save the current image as a reference for the next callback
        self.last_color_image = np.copy(self.color_image)
