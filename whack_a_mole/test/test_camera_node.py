...
from launch_testing.util import WaitForTopics
import rclpy
...
def generate_test_description():
    camera_node = Node(
        ...
        remappings=[
            ('/camera/camera/aligned_depth_to_color/image_raw', 'test_depth_image'),
...
class TestCameraNode(unittest.TestCase):
    ...
    # --------------- Begin Citation [1] -------------------#
    def test_camera_node_startup(self, launch_service, camera_node, proc_output):
        """Test that the Camera node starts and subscribes to topics."""
        # Wait for the Camera node to subscribe to the expected topics
        topic_checker = WaitForTopics(["test_depth_image", "test_color_image"], timeout=5.0, node=self.node)
        self.assertTrue(topic_checker.wait(), "Camera node did not subscribe to expected topics.")

    def test_toggle_tf_publish_service(self, launch_service, camera_node, proc_output):
        """Test the toggle_tf_publish service of the Camera node."""
        client = self.node.create_client(Empty, 'toggle_tf_publish')

        # Wait for service to be available
        self.assertTrue(client.wait_for_service(timeout_sec=5.0), "Service toggle_tf_publish not available.")
        ...
        self.assertIsNotNone(future.result(), "Service call to toggle_tf_publish failed.")
    ...
    def test_image_processing(self, launch_service, camera_node, proc_output):
        ...
        pub = self.node.create_publisher(Image, 'test_color_image', QoSProfile(depth=10))
        test_image = Image()
        ...
        while not processed_image_received and self.node.get_clock().now().seconds < end_time:
            rclpy.spin_once(self.node, timeout_sec=0.1)

        self.assertTrue(processed_image_received, "Processed image was not published.")
        # --------------- End Citation [1] -------------------#
